...
class TracksProcessor{
    ...
    static processPath(dbData){
        ...
        //process array of tracker_ids and rows into array of objects with tracker_id and results array

        var result = dbData.map((trackerData) => {
            return {
                tracker_id: trackerData.tracker_id,
                points: trackerData.results.map((item) => {
                    return {
                        time: item.time,
